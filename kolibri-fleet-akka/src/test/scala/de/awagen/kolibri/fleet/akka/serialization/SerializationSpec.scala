...
import de.awagen.kolibri.base.processing.modifiers.ParameterValues.{MappedParameterValues, ParameterValueMappingDefinition, ParameterValuesDefinition, ValueSeqGenDefinition, ValueType}
...
class SerializationSpec extends KolibriTypedTestKitNoCluster(ConfigOverwrites.configSettings) {
  ...
  "Serialization" must {
    ...
    "work properly on ParameterValueMappings" in {
      // given
      val original = new ParameterValueMappingDefinition(keyValues = parameterValues, mappedValues = Seq(mappedValue1, mappedValue2),
    ...
    "serialization of job message across actors" in {
      val parsed: SearchEvaluationDefinition = Samples.jobSample.parseJson.convertTo[SearchEvaluationDefinition]
    ...
    "serialize job message with file loads and node storage retrieval" in {
      Samples.otherJobSample.parseJson.convertTo[SearchEvaluationDefinition]
    }
