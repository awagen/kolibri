...
![Alt text](images/kolibri.svg?raw=true "Kolibri-Fleet-Akka")
...
- build jar (from root folder; find it in target folder of kolibri-fleet-akka sub-folder afterwards): ```./scripts/buildKolibriFleetAkkaJar.sh```
- build docker image for local usage (kolibri-fleet-akka sub-folder): ```docker build . -t kolibri-fleet-akka:0.1.5```
- run single-node cluster (kolibri-fleet-akka subfolder; compute and httpserver role, access via localhost:
...
- start local 3-node cluster (kolibri-fleet-akka subfolder; one compute and httpserver node, two 'compute'-only nodes, access via localhost:
...
ClusterSharding can be activated with USE_SHARDING_AND_ENDPOINTS environment variable (see docker-compose.yml in kolibri-fleet-akka).
...
The kolibri-fleet-akka code is licensed under APL 2.0.