...
object ParameterValues {
   ...
   *
   */
  class ParameterValueMapping(private[this] var keyValues: ParameterValues,
                              private[this] val mappedValues: Seq[MappedParameterValues],
                              private[this] val mappingKeyValueAssignments: Seq[(Int, Int)]) extends IndexedGenerator[Seq[ParameterValue]] with ValueSeqGenProvider {
    keyValues = removeTopLevelKeysWithMissingMappings(keyValues)
    ...
    // to generate all combinations, we generate for each of the values and generate the resulting values
    // one after another
    val allValuesGenerator: IndexedGenerator[Seq[ParameterValue]] = OneAfterAnotherIndexedGenerator(
    ...
    // explicitly define partitions such that partitioning strictly happens by the values and not by
    // the mapped values
