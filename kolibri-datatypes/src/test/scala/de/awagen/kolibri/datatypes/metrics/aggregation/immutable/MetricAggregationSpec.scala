  ...
  "OverallAggregationSpec" must {
    ...
    "correctly add all metric results" in {
      ...
      val aggregation = MetricAggregation.empty[Tag](identity)
        .addResults(Set(StringTag("test1")), metricRecord1)
        .addResults(Set(StringTag("test2")), metricRecord2)
        .addResults(Set(StringTag("test2")), metricRecord3)
        .addResults(Set(StringTag("test3")), metricRecord4)
    ...
    "correctly add other MetricAggregation" in {
      //given
      val aggregation1 = MetricAggregation.empty[Tag](identity).addResults(Set(StringTag("test1")), metricRecord1)
      ...
      val expectedR1: MetricDocument[Tag] = MetricDocument.empty[Tag](StringTag("test1")).add(metricRecord1)
