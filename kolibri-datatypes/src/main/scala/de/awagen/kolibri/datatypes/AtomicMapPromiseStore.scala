 ...
 * could potentially be expensive (e.g in case multiple experiment batches processing
 * on a node on a single node try to request the resource at once)
 ...
 * @tparam W - type of the object used to retrieve a value for a given key. Note that
 *           we also have the mapping given by retrievalObjectToKeyFunc, which in case
 *           the object used to identify how to retrieve a value is also used as key,
 *           retrievalObjectToKeyFunc needs to be set to identity
 */
trait AtomicMapPromiseStore[U, V, W] {
   ...
   */
  private[this] def loadAndStoreValues(retrievalObj: W, default: Option[SerializableSupplier[V]] = None)(implicit ec: ExecutionContext): Promise[V] = {
    val key = retrievalObjectToKeyFunc(retrievalObj)
    ...
    while (!currentMap.keySet.contains(key)) {
      ...
      if (didSetPromise) valueMap.get()(key).completeWith(Future {
        default.map(x => x.apply()).getOrElse(calculateValue(retrievalObj))
      })
   ...
   */
  def retrieveValue(retrievalObj: W, default: Option[SerializableSupplier[V]] = None)(implicit ec: ExecutionContext): Promise[V] = {
    val key: U = retrievalObjectToKeyFunc(retrievalObj)
    if (!valueMap.get().contains(key)) loadAndStoreValues(retrievalObj, default)
