...
import spray.json.DefaultJsonProtocol.{BooleanJsonFormat, DoubleJsonFormat, FloatJsonFormat, IntJsonFormat, StringJsonFormat, immSeqFormat}
...
object JsonTypeCast extends Enumeration {
  ...
  case class Val(typeName: String) extends super.Val {
    def toNamedClassType(name: String): NamedClassTyped[_] = typeName match {
      case "INT" => NamedClassTyped[Int](name)
      ...
      case "SEQ[INT]" => NamedClassTyped[Seq[Int]](name)
    ...
    def cast(value: spray.json.JsValue): Any = typeName match {
      ...
      case "SEQ[INT]" => value.convertTo[Seq[Int]]
      ...
      case _ => new RuntimeException(s"value '$value' does not conform to type corresponding to typeName $typeName")
    }
  }
