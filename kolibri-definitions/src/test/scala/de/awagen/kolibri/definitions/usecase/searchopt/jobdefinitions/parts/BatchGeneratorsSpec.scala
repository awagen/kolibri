...
import de.awagen.kolibri.definitions.processing.modifiers.ParameterValues.{MappedParameterValues, ParameterValueMappingDefinition, ParameterValuesDefinition, ValueType}
import de.awagen.kolibri.definitions.processing.modifiers.RequestTemplateBuilderModifiers.{CombinedModifier, RequestParameterModifier}
...
class BatchGeneratorsSpec extends UnitTestSpec {
  ...
  "BatchGenerators" must {
    ...
    "correctly apply batchByGeneratorAtIndex for mapping generator" in {
      import de.awagen.kolibri.definitions.processing.modifiers.ParameterValues.ParameterValuesImplicits.{ParameterValueSeqToRequestBuilderModifier, ParameterValueToRequestBuilderModifier}
      // given
      val batchByFirst = BatchGenerators.batchByGeneratorAtIndex(0)
      val batchBySecond = BatchGenerators.batchByGeneratorAtIndex(1)
