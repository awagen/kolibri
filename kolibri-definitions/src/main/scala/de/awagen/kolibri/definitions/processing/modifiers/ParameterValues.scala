...
import de.awagen.kolibri.definitions.processing.modifiers.ParameterValues.ParameterValueMappingDefinition.getGeneratorsByValues
import de.awagen.kolibri.definitions.processing.modifiers.RequestTemplateBuilderModifiers.{CombinedModifier, RequestTemplateBuilderModifier}
...
object ParameterValues {
  ...
  object ParameterValueMappingDefinition {
    ...
    private[modifiers] def getFromToMappingIndices(mappedValues: Seq[MappedParameterValues],
                                                   mappingKeyValueAssignments: Seq[(Int, Int)]): Seq[(Int, Int)] = {
      mappedValues.indices.map(mappedIndex => {
     ...
     * @return
     */
    private[modifiers] def generateValuesWithIndexedBackwardReferences(pValue: ParameterValue,
                                                                       mappedValues: Seq[MappedParameterValues],
                                                                       mappingKeyValueAssignments: Seq[(Int, Int)]): IndexedGenerator[Seq[ParameterValue]] = {

      val fromToMappingIndices: Seq[(Int, Int)] = getFromToMappingIndices(mappedValues, mappingKeyValueAssignments)
    ...
    private[modifiers] def getGeneratorsByValues(keyValues: ParameterValuesDefinition,
      ...
      cleanedKeyValues.mapGen(x => {
        if (mappingKeyValueAssignments.isEmpty) {
          ...
          generateValuesWithIndexedBackwardReferences(x, mappedValues, mappingKeyValueAssignments)
        }
      })
    }
     ...
     * @return
     */
    private[modifiers] def generateValuesFromKeyValue(keyValue: ParameterValue,
                                                      mappedValues: Seq[MappedParameterValues]): IndexedGenerator[Seq[ParameterValue]] = {
