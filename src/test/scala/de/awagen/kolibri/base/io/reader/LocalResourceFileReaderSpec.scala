...
class LocalResourceFileReaderSpec extends UnitTestSpec {
  ...
  "LocalResourceFileReader" must {
    ...
    "correctly read entries" in {
      val fileReader: LocalResourceFileReader = LocalResourceFileReader(delimiterAndPosition = None)
    ...
    "correctly read entries and pick nth after split" in {
      val fileReader: LocalResourceFileReader = LocalResourceFileReader(delimiterAndPosition = Some(("\\s+", 1)))
