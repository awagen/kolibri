...
case class MetricDocument[A <: AnyRef](id: A, rows: mutable.Map[ParamMap, MetricRow]) {
  ...
  def totalSuccessCountSum: Double = rows.values.map(x => x.totalSuccessCountSum).sum
  ...
  def totalSuccessCountMax: Double = rows.values.map(x => x.totalSuccessCountMax).max

  def totalSuccessCountMin: Double = rows.values.map(x => x.totalSuccessCountMin).min

  // sums up totalErrorCountSum over all values in the result map (MetricRow values)
  def totalErrorCountSum: Double = rows.values.map(x => x.totalErrorCountSum).sum
  ...
  def totalErrorCountMax: Double = rows.values.map(x => x.totalErrorCountMax).max

  def totalErrorCountMin: Double = rows.values.map(x => x.totalErrorCountMin).min
