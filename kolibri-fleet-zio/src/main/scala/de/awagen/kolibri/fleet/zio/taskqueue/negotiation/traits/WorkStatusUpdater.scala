...
/**
 * Takes care of picking the work status from all workers and update the proccesing state information for all.
 */
trait WorkStatusUpdater {

  /**
   * Update the processing status information for each task
   */
  def update(): Task[ProcessUpdateStatus]
