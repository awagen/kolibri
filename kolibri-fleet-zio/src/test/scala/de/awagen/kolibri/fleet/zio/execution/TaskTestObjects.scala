...
object TaskTestObjects {
  ...
  def taggerForRequestAndParseTask: ProcessingMessage[WeaklyTypedMap[String]] => ProcessingMessage[WeaklyTypedMap[String]] = msg => {
    val requestTemplate = msg.data.get(RequestJsonAndParseValuesTask.requestTemplateKey.name).get.asInstanceOf[RequestTemplate]
